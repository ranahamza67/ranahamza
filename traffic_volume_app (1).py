...
# Prediction Function
def predict_with_model(input_features):
    """
    Predict traffic volume using the trained model.

    Parameters:
        input_features (list): Feature values in the same order as 'features'.

    Returns:
        float: Predicted traffic volume.
    """
    prediction = model.predict([input_features])
    return f"Predicted traffic volume: {prediction[0]:.2f}"

# Example prediction:
input_features = [14, 2, 1, 25, 60, 12, 5, 10, 30, 0, 0, 75]  # Replace with actual feature values
print(predict_with_model(input_features))
